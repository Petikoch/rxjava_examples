...
public class InfiniteLaneMcDrive {
    ...
    public static void main(String[] args) throws InterruptedException {
        ...
        AtomicInteger numberOfConcurrentHandledCustomers = new AtomicInteger(0);

        Observable<CustomerWithArrivalTime> orderFinishedStream = customerArriveStream
                .flatMap(customerWithArrivalTime -> {

                            sysout(numberOfConcurrentHandledCustomers.incrementAndGet() + " concurrent customer(s) (=number of lanes at the moment)");
                            ...
                            Single<CustomerWithArrivalTime> finishedOrder = Single.zip(mac, fries, coke, (s, s2, s3) -> {
                                sysout(numberOfConcurrentHandledCustomers.decrementAndGet() + " concurrent customer(s)");
